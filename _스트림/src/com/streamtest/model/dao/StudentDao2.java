...
public class StudentDao2 {
	...
	static {
		for(int i=0;i<100;i++) {
			students.add(new Student.Builder().stdNo(i).name(""+(char)(Math.random()*'芌'+'啊')+(char)(Math.random()*'芌'+'啊')+(char)(Math.random()*'芌'+'啊'))
			...
			.grade((int)(Math.random()*6+1)).address("林家"+i).build());
