...
public class StudentDao {
	...
	static {
		for(int i=0;i<100;i++) {
			students.add(new Student.Builder().stdNo(i).name(""+(char)(Math.random()*('ÆR'-'°¡')+'°¡')+(char)(Math.random()*('ÆR'-'°¡')+'°¡')+(char)(Math.random()*('ÆR'-'°¡')+'°¡'))
			...
			.grade((int)(Math.random()*6+1)).address("ÁÖ¼Ò"+i).build());
